...
export default function QuotationForm({ prefill }: { prefill?: QuotationPrefill }) {
  ...
  const formSections: Array<{ title: string; fields: Array<[keyof typeof form, string]> }> = [
    { title: "Customer & site", fields: [["customerName", "Customer name"], ["customerPhone", "Phone"], ["customerEmail", "Email"], ["customerCompany", "Company / business"], ["address", "Service address"], ["city", "City"]] },
    { title: "Treatment scope", fields: [["propertyType", "Property type"], ["area", "Area / size"], ["visitFrequency", "Visit frequency"], ["numberOfVisits", "Number of visits"], ["treatmentDescription", "Treatment description"]] },
  ];
  const summaryFields: Array<[string, "discount" | "taxRate" | "additionalCharges"]> = [["Discount ₹", "discount"], ["GST / Tax %", "taxRate"], ["Additional charges ₹", "additionalCharges"]];
  ...
  return <form onSubmit={submit} className="grid gap-6 xl:grid-cols-[1fr_360px]">
    <div className="space-y-5">
      ...
      {formSections.map(({ title, fields }) => <section className={`${ui.card} p-6`} key={title}><h2 className="text-lg font-semibold">{title}</h2><div className="mt-5 grid gap-4 md:grid-cols-2">{fields.map(([key, label]) => <label className={key === "address" || key === "treatmentDescription" ? "md:col-span-2" : ""} key={key}><span className={ui.label}>{label}</span>{key === "treatmentDescription" ? <textarea className={`${ui.input} mt-1 h-28 py-3`} value={form[key]} onChange={(e) => set(key, e.target.value)} /> : <input className={`${ui.input} mt-1`} type={key === "area" || key === "numberOfVisits" ? "number" : "text"} value={form[key]} onChange={(e) => set(key, e.target.value)} required={key === "customerName" || key === "customerPhone" || key === "address"} />}</label>)}</div><label className="mt-4 block"><span className={ui.label}>Service type</span><select className={`${ui.input} mt-1`} value={form.serviceType} onChange={(e) => set("serviceType", e.target.value)}>{Object.entries(SERVICE_TYPE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></section>)}
