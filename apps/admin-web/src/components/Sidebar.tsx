...
const NAV_ITEMS = [
  ...
  { label: "Technician Tracking", href: "/dashboard/tracking", icon: "◉" },
  { label: "Mileage Report", href: "/dashboard/mileage", icon: "▲" },
...
export function Sidebar({ user }: SidebarProps) {
  ...
  return (
    <>
      {/* Hamburger toggle — mobile only. Desktop (md+) keeps the
          sidebar always visible, exactly as before. */}
      <button
        type="button"
      ...
      <nav
        aria-label="Primary"
        className={
          "fixed inset-y-0 left-0 z-50 flex w-64 shrink-0 flex-col overflow-y-auto bg-surface border-r border-border-default transition-transform duration-200 ease-out " +
          "md:static md:z-auto md:translate-x-0 md:min-h-dvh " +
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-6">
          <div>
            <p className="font-mono text-xs tracking-wide text-accent">PEST MANTRA</p>
            <h2 className="mt-1 text-[17px]">Field Console</h2>
          </div>
          <button
            type="button"
            onClick={() => setIsMobileOpen(false)}
        ...
        <ul className="flex-1 space-y-1 px-3">
          {NAV_ITEMS.map((item) => {
            ...
            // everything else, so /dashboard/projects/123 still
            // highlights the "Projects" nav item.
            const isActive =
        ...
        <div className="border-t border-border-default px-5 py-4 text-xs text-ink-faint">
          ...
          <p className="mt-0.5 font-medium text-ink-muted">
            {user.role === UserRole.SUPER_ADMIN ? "Super Admin" : "Office Admin"}
          </p>
        </div>
      </nav>
    </>
