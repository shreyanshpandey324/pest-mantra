...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const data = await backendFetch<{ checkouts: ChemicalCheckout[] }>("/chemicals/checkout/open", {
      accessToken: token,
    });
    ...
    return NextResponse.json(
      {
        success: false,
        message: err instanceof BackendApiError ? err.message : "Failed to load open checkouts",
      },
